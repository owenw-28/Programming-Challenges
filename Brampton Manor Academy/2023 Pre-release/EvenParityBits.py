...
def SetFlags(Value, Registers):
  ...
  if Registers[ACC] >= 0:
    if ConvertToBinary(Registers[ACC]).count("1") % 2 == 0:
      ...
      Registers[STATUS] = ConvertToDecimal(ConvertToBinary(Registers[STATUS]) + "0")
  if Registers[ACC] < 0:
    Temp = TwosComplement(Registers[ACC])
    if ConvertToBinary(Temp).count("1") % 2 == 0:
      Registers[STATUS] = ConvertToDecimal(ConvertToBinary(Registers[STATUS]) + "1")
    else:
      Registers[STATUS] = ConvertToDecimal(ConvertToBinary(Registers[STATUS]) + "0")
  return Registers

def TwosComplement(NegativeNum):
  PositiveNum = NegativeNum * -1
  UnsignedBinaryPattern = ConvertToBinary(PositiveNum)
  SwappedBits = ''
  while len(UnsignedBinaryPattern) < 8:
