...
def UpdateSymbolTable(SymbolTable, ThisLabel, LineNumber):
  if ThisLabel in SymbolTable:
    print("Error: Duplicate labels have been used")
  else:
...
def ExtractLabel(Instruction, LineNumber, Memory, SymbolTable):
  if len(Instruction) > 0: 
    ...
    if ThisLabel != EMPTY_STRING:
      if Instruction[5] != ':':
        print("Error: Colon should always be the sixth character in instruction")
        Memory[0].OpCode = "ERR"
      else:
...
def Execute(SourceCode, Memory): 
  ...
  while OpCode != "HLT":
    ...
    if OpCode == "LDA":
      ...
      Memory, Registers = ExecuteJSR(Memory, Registers, Operand, SourceCode)
    elif OpCode == "CMP#":
  ...
  while not Finished:
    ...
    if MenuOption == 'L':
      ...
      if Memory[0].OperandValue == 0:
        print("Error: Code has not been assembled")
      elif Memory[0].OpCode == "ERR":  
        print("Error: There is an error in the code")
      else:
