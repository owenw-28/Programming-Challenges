...
def ExtractOpCode(Instruction, LineNumber, Memory):
  if len(Instruction) > 9:
    OpCodeValues = ["LDA", "STA", "LDA#", "HLT", "ADD", "JMP", "SUB", "CMP#", "BEQ", "SHL", "SHR", "SKP", "JSR", "RTN", "   "]
...
def ExecuteSHL(Registers, Operand):
  Registers[ACC] *= 2**Operand
  Registers = SetFlags(Registers[ACC], Registers)
  if Registers[STATUS] == ConvertToDecimal("001"):
    ReportRunTimeError("Overflow", Registers)
  return Registers

def ExecuteSHR(Registers, Operand):
  Registers[ACC] /= 2**Operand
  if type(Registers[ACC]) is not int:     #Round down if float
    Registers[ACC] = int(Registers[ACC] - 0.5) if Registers[ACC] > 0 else int(Registers[ACC] + 0.5)
  Registers = SetFlags(Registers[ACC], Registers)
  if Registers[STATUS] == ConvertToDecimal("001"):
    ReportRunTimeError("Overflow", Registers)
  return Registers
...
def Execute(SourceCode, Memory): 
  ...
  while OpCode != "HLT":
    ...
    if OpCode == "LDA":
      ...
      Registers = ExecuteSHL(Registers, Operand)
    elif OpCode == "SHR":
      Registers = ExecuteSHR(Registers, Operand)
    elif OpCode == "SUB":
