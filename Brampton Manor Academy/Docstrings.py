...
def LoadFile(SourceCode):
  ...
  try:
    ...
    if not FileExists:
      ...
      print("Error Code 2")     #If there are any other errors with the file
  ...
  if len(Instruction) > 9:
    ...
    if Operation in OpCodeValues:
      ...
      if Operation != EMPTY_STRING:
        print("Error Code 5")     #If there is a problem with the opcode (does not hold value from OpCodeValues)
        Memory[0].OpCode = "ERR"
  return Memory   
    ...
    if Operand != EMPTY_STRING:
      if Operand in SymbolTable:
        ...
        try:
          ...
          print("Error Code 6")     #If there is a problem with the operand (cannot be casted into an integer)
          Memory[0].OpCode = "ERR"
  return Memory
  ...
  while not Finished:
    ...
    if MenuOption == 'L':
      ...
      if SourceCode[0] == EMPTY_STRING:
        print("Error Code 7")     #If there is nothing in SourceCode[0] (not loaded file)
      else:
        DisplaySourceCode(SourceCode)
    elif MenuOption == 'E':
      if SourceCode[0] == EMPTY_STRING:
        print("Error Code 8")     #If there is nothing in SourceCode[0] (not loaded file)
      else:
        SourceCode = EditSourceCode(SourceCode)
        Memory = ResetMemory(Memory)
    elif MenuOption == 'A':
      if SourceCode[0] == EMPTY_STRING:
        print("Error Code 9")     #If there is nothing in SourceCode[0] (not loaded file)
      else:
        Memory = Assemble(SourceCode, Memory)
    elif MenuOption == 'R':
      if Memory[0].OperandValue == 0:
        print("Error Code 10")     #If there is no change in the value stored in Memory[0]
      elif Memory[0].OpCode == "ERR":  
        print("Error Code 11")     #If there is an error in the instructions
      else:
