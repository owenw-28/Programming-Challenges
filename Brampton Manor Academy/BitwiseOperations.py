...
def ExtractOpCode(Instruction, LineNumber, Memory):
  if len(Instruction) > 9:
    OpCodeValues = ["LDA", "STA", "LDA#", "HLT", "ADD", "JMP", "SUB", "CMP#", "BEQ", "AND", "OR", "XOR", "NOT", "SKP", "JSR", "RTN", "   "]
...
def TwosComplement(UnsignedBinaryPattern):
    SwappedBits = ''
    UnsignedBinaryPattern = '0' + UnsignedBinaryPattern
    for Bit in UnsignedBinaryPattern:
      if Bit == '1':
        SwappedBits += '0'
      else:
        SwappedBits += '1'
    Temp = ConvertToDecimal(SwappedBits)
    Temp += 1
    return Temp
...
def ExecuteAND(Memory, Registers, Address):
  Operand = Memory[Address].OperandValue
  if Registers[ACC] < 0:
    Registers[ACC] *= -1
    BinaryPattern = ConvertToBinary(Registers[ACC])
    Registers[ACC] = TwosComplement(BinaryPattern)
  if Operand < 0:
    Operand *= -1
    BinaryPattern = ConvertToBinary(Operand)
    Operand = TwosComplement(BinaryPattern)
  Registers[ACC] = Registers[ACC] & Operand
  Registers = SetFlags(Registers[ACC], Registers)
  return Registers

def ExecuteOR(Registers, Operand):
  Operand = Memory[Address].OperandValue
  if Registers[ACC] < 0:
    Registers[ACC] *= -1
    BinaryPattern = ConvertToBinary(Registers[ACC])
...
def ExecuteXOR(Registers, Operand):
  Operand = Memory[Address].OperandValue
  if Registers[ACC] < 0:
    Registers[ACC] *= -1
    BinaryPattern = ConvertToBinary(Registers[ACC])
...
def Execute(SourceCode, Memory): 
  ...
  while OpCode != "HLT":
    ...
    if OpCode == "LDA":
      ...
      Registers = ExecuteXOR(Memory, Registers, Operand)
    elif OpCode == 'NOT':
