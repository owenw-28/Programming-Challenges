...
def SaveFile(SourceCode):
  ...
  if exists(f"{FileName}.txt"):
    Choice = input("File already exists, do you want to overwrite the existing file? {y/n)? ")
    if Choice == "n":
      print("The file has not been saved")
    else:
      FileOut = open(f"{FileName}.txt", 'w')
      for Address in range(1, int(SourceCode[0])):
        FileOut.write(SourceCode[Address])
        FileOut.write('\n')
      FileOut.close()
  else:
    FileOut = open(f"{FileName}.txt", 'w')
